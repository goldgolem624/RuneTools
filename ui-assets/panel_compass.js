...
// Spliced inline into client.html; bare classic script sharing one global scope.
  ...
  let compassMapSig = null;       // signature of the last-drawn compass map (skip redundant terrain refetch)
  let compassNote = '';
  ...
  async function drawCompassMap() {
    ...
    if (ext <= 160 && bridge() && bridge().mapWindow) {
      ...
      if (meta && meta.b64) { try { const bin = atob(meta.b64), a = new Uint8ClampedArray(bin.length); for (let i = 0; i < bin.length; i++) a[i] = bin.charCodeAt(i); g.putImageData(new ImageData(a, W, W), 0, 0); clueDrawNomove(g, meta); clueDrawObjects(g, meta); clueDrawTeleports(g, meta); clueDrawLabelsWindow(g, meta); drewTerrain = true; } catch (e) {} }
