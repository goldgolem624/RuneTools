...
(function () {
  ...
  async function fetchGlobetrotter() {
    ...
    try {
      let eq = null; try { eq = JSON.parse(await bridge().containerItems(myPid(), 94)); } catch (e) {}
      const worn = {}, list = (eq && Array.isArray(eq.items)) ? eq.items : [];
      for (const it of list) {
        const id = Array.isArray(it) ? (it[1] | 0) : 0; if (id <= 0) continue;
        let nm = GT_NAMES[id];
        if (nm === undefined) { try { nm = (JSON.parse(await bridge().itemInfo(id)).name) || ''; } catch (e) { nm = ''; } GT_NAMES[id] = nm; }
  ...
  async function renderClueStats() {
    ...
    try { if (bridge() && bridge().varps) clueStatsVp = JSON.parse(await bridge().varps(myPid(), '7802,7803,7804,7805,7806')); } catch (e) {}
...
registerTab({ id: 'clues', render: renderClues, open: function () { clueListSig = ''; fetchClues(true); }, close: function () { try { activeClueId = -1; if (bridge() && bridge().guideMarks) bridge().guideMarks(myPid(), ''); clueSetNpc(''); } catch (e) {} } });
