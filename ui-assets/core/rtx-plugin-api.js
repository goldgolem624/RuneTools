...
try{parent.postMessage({__rtxPlugin:P,kind:'hello'},'*');}catch(e){}})();`;
  ...
  const PLUGIN_API = {
    ...
    'state.nexus':      { scope: 'state.read', json: true,    run: (a, pid) => (bridge().nexusItems ? bridge().nexusItems(pid) : '{}') },
    ...
    'state.social':     { scope: 'state.read', json: true,    run: (a, pid) => bridge().social(pid) },
    'state.combatLog':  { scope: 'state.read', json: true,    run: (a, pid) => bridge().combatLog(pid, pClampNum(a[0], 0, 9007199254740991), pClampNum(a[1], 1, 2000) | 0) },
    'state.walkable':   { scope: 'state.read', json: true,    run: (a) => bridge().walkGrid(pClampNum(a[0], 1, 16383) | 0, pClampNum(a[1], 1, 16383) | 0, pClampNum(a[2], 0, 3) | 0, pClampNum(a[3], 1, 8) | 0) },
    ...
    // The instance you are in right now, already decoded: name, mode and live health.
    // Raw sources are varp 10946/10950 and varbits 53292/53294 (see docs/instance-mode.md), but the
    // per-encounter mode rules are knowledge, not data, so we hand plugins the answer instead.
    'state.encounter':  { scope: 'state.read', json: false,   run: async (a, pid) => {
                            let vp = {}; try { vp = JSON.parse(await bridge().varps(pid, ENC_VARPS)); } catch (e) {}
                            const sid = vp['10946'] | 0, mode = vp['10950'] | 0;
                            if (sid <= 0) return null;                       // -1 when not in an encounter
                            let name = '', tenth = false;
                            if (bridge().structParams) {
                              try {
                                const sp = JSON.parse(bridge().structParams(sid) || 'null');
                                if (sp) { name = (sp.strs && sp.strs['8849']) || '';
                                          tenth = ((sp.ints && sp.ints['8867']) | 0) === 1; }
                              } catch (e) {}
                            }
                            if (!name) return null;                          // not an encounter struct
                            const vb = await readVarbitValues([53292, 53294, 27168, 50177]) || {};
                            const d = (tenth && (vb[27168] | 0) === 1) ? 10 : 1;
                            return { struct: sid, name, mode: encLabel(sid, mode, vb[50177] | 0) || null,
                                     modeValue: mode,
                                     health: ((vb[53292] | 0) / d) | 0, healthMax: ((vb[53294] | 0) / d) | 0 }; } },
    ...
    'state.clientState': { scope: 'state.read', json: true,    run: (a, pid) => bridge().clientState(pid) },
    ...
    'cache.varbitDomainMap': { scope: 'cache.read', json: true, run: () => (bridge().varbitDomainMap ? bridge().varbitDomainMap() : '{}') },
    ...
    'notify.discord':   { scope: 'notify.discord', json: 'maybe', run: (a, pid, id) => (bridge().discordNotify ? bridge().discordNotify(pClampStr(a[0], 1000), 'plugin:' + String(id || '').slice(0, 40)) : null) },
    ...
    'overlay.guideTiles':{ scope: 'overlay', json: false, run: (a, pid) => {
                            ...
                            for (const m of marks.slice(0, 64)) {
                              ...
                              const snapId = pClampNum(m.snapId, 0, 300000) | 0;   // the loc the mark names
                              const snap = (m.snap || snapId) ? 1 : 0;
                              if (hasArea || col || col2 || merge || snap || snapId)
                                rec += '\x1f' + snap + '\x1f' + col + '\x1f' + (hasArea ? x2 : 0) + '\x1f' + (hasArea ? y2 : 0) + '\x1f' + merge + '\x1f' + col2 + '\x1f' + snapId;
                              recs.push(rec);
                            }
