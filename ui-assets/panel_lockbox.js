...
(function () {
  ...
  let lockboxBusy = false; let lockboxTarget = null; lockboxOwnsPanel = false; let lockboxDrawSig = ''; let lockboxHl = false;
  function lockboxClearHl() { if (!lockboxHl) return; try { if (bridge() && bridge().puzzleCells) rtxData.sync('solver.puzzleCells', ''); } catch (e) {} lockboxHl = false; }
  ...
  function lockboxHighlight(order, clicks, comps, abs) {
    ...
    if (segs.length) { rtxData.sync('solver.puzzleCells', segs.join(';')); lockboxHl = true; } else lockboxClearHl();
  }

// ---- IIFE exports (generated by panel_iife.py: only names other files use) ----
Object.assign(window, { lockboxTick });
})();
