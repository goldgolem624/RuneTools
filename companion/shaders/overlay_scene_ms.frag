...
void main() {
    ...
    if ((pc.mode & 16) != 0) at.y = textureSize(u_depth).y - 1 - at.y;
    float d = texelFetch(u_depth, at, 0).r;
    if ((pc.mode & 8) != 0) {
    ...
    if ((pc.mode & 2) != 0 && v_z >= 0.0 && pc.b != 0.0) {
        ...
        if (abs(qm) > 1e-9) {
            ...
            float hidden = 0.0;
            int n = min(textureSamples(u_depth), 8);
            for (int i = 0; i < n; ++i) {
                float qs = texelFetch(u_depth, at, i).r + pc.a;
