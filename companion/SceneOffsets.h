...
inline constexpr std::uint64_t kType     = 0x20;    // sec -> type byte (0x10 through 949-5: the sec header grew by 0x10 on 950-1;
                                                     // uid/name/pos and everything from +0x88 on did NOT move)
...
inline constexpr std::uint64_t kPlane    = 0x50;    // sec -> plane (0x40 through 949-5; 950-1 value only seen on plane 0 so far)
...
inline constexpr std::uint64_t kT4Size   = 0x1C0;   // type-4 (graphic highlight) object size (0x1B0 + the 0x10 header growth; assumed)
...
inline constexpr std::uint64_t kT13Size  = 0x108;   // type-13 (world marker) object size (0xF8 + the 0x10 header growth; assumed)
...
// the scene at all: the click handler (0xE1A30 on 949, 0xE3110 on 950-1) writes it straight onto ClientMiniMap
// before it sends the move packet or spawns the marker:
//     mov rax,[rcx+0x19890]        ; ClientMiniMap
...
//
// SEPARATE object, do not confuse the two: root+0x198D0 is the walk-marker APPEARANCE
// manager. CS2 op SETWALKMARKER (scrambled 2222 on 949, 308 on 950-1) writes it via 0x14019f2b0 (0x140128d90 on 950-1), and the only
