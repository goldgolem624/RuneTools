...
import { ClientScriptOp, ImmediateType, StackConstants, StackDiff, StackInOut, StackList, StackType, namedClientScriptOps, variableSources, typeToPrimitive, getOpName, knownClientScriptOpNames, subtypes, dynamicOps, pushConstantIntOp } from "./definitions";
import { groundTruthSignatures } from "./generated_opsignatures";
...
var varbitInfoParser = new FileParser<{ varid: number, bits: [number, number] }>({
    ...
    "0x10": { "name": "0x10", "read": true }//build ~948+ flag, 0 bytes (verified: all 60988 varbit defs parse clean)
});
...
export class OpcodeInfo {
    ...
    //true when stackinfo comes from a hand-callibrated declaration instead of the
    //solver, the solver is not allowed to trim or replace a pinned signature
    pinned = false;
...
// Hand callibrated op signatures. Two kinds of entries:
// - ops the game added (or re-typed) after the openrs2 reference caches: the
//   reference bounce can only resolve an op when it appears byte-identical inside a
//   matched reference script, anything newer falls through to a generated
//   unk<opidcounter> id (build-local, shifts every update)
// - stable-id ops whose signatures the stack count solver gets wrong or unstable,
//   typically because an unmodeled in/out pair cancels in every section total
// All identified by hand from render corpus analysis. Every entry is
// gated on witness scripts (live script ids known to use the op) so a future op-table
// shift can't silently apply a signature to the wrong op.
type HandCallibratedOp = {
    id: number,
    name: string,
    in: number[],//subtypes ids in push order, unknown_* for generic values
    out: number[],
    witnesses: number[]
};
const postReferenceOps: HandCallibratedOp[] = [
    //native text input api
    { id: 10995, name: "TEXTINPUT_SETPLACEHOLDER", in: [subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_int, subtypes.component], out: [], witnesses: [20891, 20858, 15015] },
    { id: 10996, name: "TEXTINPUT_BLUR", in: [], out: [], witnesses: [20869, 20892, 16266] },
    { id: 10997, name: "TEXTINPUT_ISFOCUSED", in: [], out: [subtypes.boolean], witnesses: [20893, 20869, 20870] },
    { id: 10999, name: "TEXTINPUT_FOCUS", in: [subtypes.component], out: [], witnesses: [15018, 15031] },
    //same op as reference id 10622 MIN_LONG, it no longer matches because the op table shifted
    { id: 11002, name: "MIN_LONG", in: [subtypes.unknown_long, subtypes.unknown_long], out: [subtypes.unknown_long], witnesses: [16795, 20458] },
    //first arg is an int (script10495 return, enum 9183 lookup), not a text string
    { id: 11051, name: "CC_SETTOOLTIP_DISABLED", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.component], out: [], witnesses: [20887] },
    //64bit successor of STOCKMARKET_GETOFFERPRICE (10599) which returned int
    { id: 11062, name: "STOCKMARKET_GETOFFERPRICE_LONG", in: [subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_long], witnesses: [20890, 621, 651] },

    //multi criteria db query family (classic ops 732-738, stable rsmv ids, named in
    //opnames.ts). Resultset handles flow through the int stack: FIND* ops produce a
    //handle from one criterion, COMBINE merges two handles, EXECUTE_COUNT consumes a
    //handle plus (packed sort column?, flag, -1, 0), returns the result count and
    //primes dbrow_findnext iteration (script19931 derives a clue number from the row's
    //position in the sorted results). The count solver can't derive the handle in/out
    //itself because they cancel in every stack total.
    { id: 10257, name: "DBQUERY_FIND_VALUE", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [19931, 13406, 16022] },
    { id: 10649, name: "DBQUERY_EXECUTE_COUNT", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [19931, 16022, 18177] },
    //unary resultset transform, exact operation unknown (13406: FIND then this before a COMBINE)
    { id: 10404, name: "DBQUERY_NOT", in: [subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [13406, 16022, 18179] },
    { id: 10364, name: "DBQUERY_AND", in: [subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [13406, 16022] },
    { id: 10385, name: "DBQUERY_FIND_STRING", in: [subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [18188, 18218, 20644] },
    //two packed columns of the same table plus an operator-like int, exact compare unknown
    { id: 10405, name: "DBQUERY_FILTER_FIELD_OP", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [13406, 18185, 18215] },
    //second scrambled opcodes of the same family added by a later update, usage
    //profiles are identical to DBQUERY_AND / DBQUERY_FIND_VALUE (build-local unk ids)
    { id: 11032, name: "DBQUERY_AND2", in: [subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [20644, 20645, 13725] },
    { id: 11069, name: "DBQUERY_FIND_VALUE2", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [20169, 3281] },

    //ops whose organic solution is unstable between callibration runs (one export
    //solved them, another degraded them), pinned at their render-corpus arity
    //OC_ICURSOR consumes the op text pushed by OC_IOP plus (obj, opindex)
    { id: 10560, name: "OC_ICURSOR", in: [subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_int], out: [], witnesses: [1520, 7031, 2833] },
    { id: 5405, name: "SPLINE_NEW", in: [subtypes.unknown_int, subtypes.unknown_int], out: [], witnesses: [2762, 2823, 2826] },
    { id: 5406, name: "SPLINE_ADDPOINT", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int], out: [], witnesses: [2762, 2823, 2826] },
    { id: 10194, name: "IF_SETOPKEY", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.component], out: [], witnesses: [20759, 19547, 20788] },
    { id: 10552, name: "IF_SETOPKEYRATE", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int, subtypes.component], out: [], witnesses: [12920, 11970, 8771] },

    //build-local unks identified from usage, semantics not fully proven
    //boolean gate for clan channel transmit handlers, arg selects own(0)/guest(1) channel
    { id: 10998, name: "CLANCHANNEL_UNK10998", in: [subtypes.unknown_int], out: [subtypes.boolean], witnesses: [3164, 3159, 17117] },
    //per component boolean query, OR'd with IF_GETHIDE==0 so not plain hide state
    { id: 11119, name: "IF_UNK11119", in: [subtypes.component], out: [subtypes.boolean], witnesses: [3159, 3799] },

    //stack() leak audit round, all shapes read off the render corpus.
    //pushes its int immediate, the count solver derived garbage for it (see also the
    //pushConstantIntOp handler in ast.ts which exposes the immediate as a constant)
    { id: 10574, name: "PUSH_CONSTANT_INT", in: [], out: [subtypes.unknown_int], witnesses: [19237, 4011, 18890] },
    //every call site pushes exactly (opindex, component)
    { id: 10193, name: "IF_SETOPKEYIGNOREHELD", in: [subtypes.unknown_int, subtypes.component], out: [], witnesses: [1309, 16019] },
    //classic filter triple: push public, push private, push trade, CHAT_SETFILTER
    { id: 5000, name: "CHAT_GETFILTER_PUBLIC", in: [], out: [subtypes.unknown_int], witnesses: [1089, 112, 11556] },
    { id: 5016, name: "CHAT_GETFILTER_TRADE", in: [], out: [subtypes.unknown_int], witnesses: [1089, 112, 11556] },
    { id: 5001, name: "CHAT_SETFILTER", in: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_int], out: [], witnesses: [1089, 112, 11556] },
    //world entry tuples, shapes from the destructuring assignments that consume them
    { id: 10485, name: "WORLDLIST_SPECIFIC", in: [subtypes.unknown_int], out: [subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_string], witnesses: [16861, 20334, 7421] },
    //same iterator pair the named ops 10827/10828 covered before the update shifted
    //the op table, tuple = WORLDLIST_SPECIFIC plus the leading world id
    { id: 11123, name: "WORLDLIST_NEXT", in: [], out: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_string], witnesses: [20332, 7420, 3067] },
    { id: 11124, name: "WORLDLIST_START", in: [], out: [subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_int, subtypes.unknown_string], witnesses: [20332, 7420, 3067] },
    //chat user details by uid (fed by CHAT_GETNEXTUID), exact field meanings unproven
    { id: 11081, name: "CHAT_UNK11081", in: [subtypes.unknown_int], out: [subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_string, subtypes.unknown_string, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_string, subtypes.unknown_int, subtypes.unknown_int], witnesses: [14941, 15598, 224] },
    //takes a single spacing value, the second int this corpus guess claimed belongs to
    //CC_GRID_SETLAYOUTPARAMS which precedes it (binary dump, definitive for the grid op;
    //the dump's own row for this op is 1 int too but was left unadjudicated there)
    { id: 10353, name: "CC_SETCHILDSPACING", in: [subtypes.unknown_int], out: [], witnesses: [13934, 15945, 7925] },
    //fine coord math used by the camera cutscene scripts, coordfine rides the string
    //stack; MOVECOORD_FINE is NOT pinned, its arity is still ambiguous in the corpus
    { id: 10399, name: "COORDZ_FINE", in: [subtypes.unknown_string], out: [subtypes.unknown_int], witnesses: [3009, 3054] },
    { id: 10400, name: "COORDX_FINE", in: [subtypes.unknown_string], out: [subtypes.unknown_int], witnesses: [3009, 3054] },
    { id: 10402, name: "COORDY_FINE", in: [subtypes.unknown_string], out: [subtypes.unknown_int], witnesses: [3009, 3054] },
    { id: 10403, name: "COORD_GRIDTOFINE", in: [subtypes.unknown_int, subtypes.unknown_int], out: [subtypes.unknown_string], witnesses: [3009, 3054] },
    { id: 10395, name: "COS_DEG", in: [subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [3009, 7193] },
    { id: 10396, name: "SIN_DEG", in: [subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [3009, 7193] },
    //long tail arity pins, each shape read off consistent spill patterns at every
    //call site of the op in the render corpus
    { id: 10354, name: "CC_PANEL_SETISVERTICAL", in: [subtypes.unknown_int], out: [], witnesses: [10026, 10028, 15945] },
    //CC_GRID_SETLAYOUTPARAMS is deliberately absent, the binary dump proves it takes 3
    //ints (isVertical, cellX, cellY) where this corpus guess said 2
    { id: 10654, name: "SHOP_GETPRODUCTCOUNT", in: [subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [15727, 15729, 16140] },
    { id: 10601, name: "NOTIFICATIONS_ISLOCALSCHEDULED", in: [subtypes.unknown_int], out: [subtypes.boolean], witnesses: [3531, 17077, 11774] },
    { id: 10889, name: "CHATCAT_GETPHRASECOUNT", in: [subtypes.unknown_int], out: [subtypes.unknown_int], witnesses: [1062, 2433] },
    //(index, label) per option, unk11188 is the update's second opcode for it
    { id: 10849, name: "CC_RADIOGROUP_ADDOPTION", in: [subtypes.unknown_int, subtypes.unknown_string], out: [], witnesses: [15966] },
    { id: 11188, name: "CC_RADIOGROUP_ADDOPTION2", in: [subtypes.unknown_int, subtypes.unknown_string], out: [], witnesses: [15966] },
    //chatcat to display string (feeds CC_SETOPBASE/SETTEXT), exact meaning unproven
    { id: 11187, name: "CHATCAT_UNK11187", in: [subtypes.unknown_int], out: [subtypes.unknown_string], witnesses: [1062] },
];

function applyPostReferenceOps(calli: ClientscriptObfuscation, checkWitnesses: boolean) {
    for (let decl of postReferenceOps) {
        let op = calli.decodedMappings.get(decl.id);
        if (!op) { continue; }
        if (checkWitnesses) {
            let witnessed = decl.witnesses.some(id => calli.candidates.get(id)?.scriptcontents?.opcodedata.some(q => q.opcode == decl.id));
            if (!witnessed) {
                console.log(`hand-callibrated op ${decl.id} (${decl.name}) skipped, no witness script uses it`);
                continue;
            }
        }
        let sig = StackInOut.fromExact(decl.in, decl.out);
        //keep an equal already-loaded signature since it may carry solved exact types
        let shapeEqual = op.stackinfo.initializedthrough
            && op.stackinfo.in.getStackdiff().equals(sig.in.getStackdiff())
            && op.stackinfo.out.getStackdiff().equals(sig.out.getStackdiff());
        if (!shapeEqual) { op.stackinfo = sig; }
        op.pinned = true;
        knownClientScriptOpNames[decl.id] = decl.name;
    }
}

// Pinned signatures (generated_opsignatures.ts). These fix how many values an op pops
// and pushes, but carry no ordering for mixed type signatures and no subtypes, so they
// are applied BEFORE the hand callibrated table above, which may refine them.
//
// Each entry records which rsmv opcode id its client opcode mapped to when the table was
// built. Client opcode numbers shift whenever the game inserts ops, so the whole table is
// gated on those pairs still matching the current callibration: a shifted op table makes
// the agreement collapse and the table is ignored rather than misapplied.
const groundTruthMinAgreement = 0.95;

function applyGroundTruthSignatures(calli: ClientscriptObfuscation) {
    let agreed = 0;
    let checked = 0;
    for (let row of groundTruthSignatures) {
        if (!row.rsmvid) { continue; }
        checked++;
        let op = calli.mappings.get(row.opcode);
        if (op && op.id == row.rsmvid) { agreed++; }
    }
    if (checked == 0 || agreed / checked < groundTruthMinAgreement) {
        console.log(`ground truth op signatures skipped, op table no longer matches the dump (${agreed}/${checked} opcodes agree)`);
        return;
    }

    //ops whose stack effect is context dependent (immediate driven, vararg or resolved
    //from constants on the stack), rsmv models these per call site and a fixed
    //signature would be wrong
    let contextualOps = new Set<number>([...Object.values(namedClientScriptOps), ...dynamicOps, pushConstantIntOp]);
    ...
    for (let row of groundTruthSignatures) {
        let op = calli.mappings.get(row.opcode);
        if (!op) { continue; }//op is not used by any script in this cache
        if (row.rsmvid && op.id != row.rsmvid) { skippedstale++; continue; }
        if (contextualOps.has(op.id)) { skippedcontext++; continue; }
        //counts only, so mixed type signatures become one unordered group, which is
        //the representation rsmv already uses for script arguments
        op.stackinfo = new StackInOut(
            new StackDiff(row.inints, row.inlongs, row.instrings).getArglist(),
            new StackDiff(row.outints, row.outlongs, row.outstrings).getArglist()
        );
        op.pinned = true;
        applied++;
    }
    console.log(`applied ${applied} ground truth op signatures (${agreed}/${checked} opcodes agree, skipped ${skippedstale} stale, ${skippedcontext} contextual)`);
}

// All SETON* interface hook ops share one vm level signature, the script pushes the
// callback script id (int), the callback arguments and a type string ("iis"), which
// the ast layer models as an [int,vararg] pair, the IF_ variants additionally take
// the target component. Many hook ops only ever occur with type strings shorter than
// 3 chars which the operant solver refuses to disambiguate from plain string
// constants (unknownstack guard in ast.ts), leaving their signature unsolved and the
// decompiler emitting bare stack() lines around every such hook site. The signature
// is uniform so pin it for every named SETON op.
const hookOpNamePattern = /^(CC|IF)_([A-Z0-9]+_)?SETON/;
function applyHookOpSignatures(calli: ClientscriptObfuscation) {
    for (let op of calli.decodedMappings.values()) {
        let name = knownClientScriptOpNames[op.id];
        if (!name || !hookOpNamePattern.test(name)) { continue; }
        let args: StackType[] = ["int", "vararg"];
        if (name.startsWith("IF_")) { args.push("int"); }//target component
        let wanted = new StackList(args).getStackdiff();
        let shapeEqual = op.stackinfo.initializedthrough
            && op.stackinfo.in.getStackdiff().equals(wanted)
            && op.stackinfo.out.getStackdiff().equals(new StackDiff());
        if (!shapeEqual) { op.stackinfo = new StackInOut(new StackList(args), new StackList()); }
        op.pinned = true;
    }
}
...
export class ClientscriptObfuscation {
    ...
    static async fromJson(source: CacheFileSource, deobjson: ReturnType<ClientscriptObfuscation["toJson"]>, scriptjson: null | ReturnType<ClientscriptObfuscation["getScriptJson"]>) {
        ...
        for (let opjson of deobjson.mappings) {
            ...
            //several scrambled opcodes can decode to the same op id, only the instance
            //the ast layer referenced got its stack solved during callibration, make
            //sure that one wins here instead of whichever the json lists last
            let prev = r.decodedMappings.get(op.id);
            if (!prev || (!prev.stackinfo.initializedthrough && op.stackinfo.initializedthrough)) {
                r.decodedMappings.set(op.id, op);
            }
        }
        r.opidcounter = deobjson.opidcounter;
        r.foundEncodings = true;
        //saved callibrations don't carry op names and may predate the hand-callibrated
        //declarations, witnesses can't be checked here since candidates aren't loaded
        applyGroundTruthSignatures(r);
        applyPostReferenceOps(r, false);
        applyHookOpSignatures(r);
        await r.preloadData();
        if (scriptjson) {
    ...
    async runCallibrationFrom(refscript: ReferenceCallibration) {
        ...
        this.parseCandidateContents();
        //pin known signatures before operant solving, both to name the ops and to stop
        //their unknown signatures from poisoning neighbouring ops that share stack
        //equations with them. binary ground truth first, hand callibration refines it
        applyGroundTruthSignatures(this);
        applyPostReferenceOps(this, true);
        applyHookOpSignatures(this);
...
function callibrateOperants(calli: ClientscriptObfuscation, candidates: Map<number, ScriptCandidate>) {
    ...
    let testSection = (eq: StackDiffEquation) => {
        ...
        //a scan that runs into an inconsistency (eg a pinned hook signature meeting a
        //call site the ast layer can't model) just stops there, like the plain break
        //cases, so the stack count equation below still gets a chance to run
        ...
        for (let i = 0; i < section.children.length; i++) {
            ...
            if (node.knownStackDiff) {
                ...
                //stop at pinned ops, their signature is already final and scanning past
                //them reaches stack states the organic solver never saw, which its
                //init/trim heuristics then misjudge for the ops that follow
                if (node.opinfo.pinned) { break; }
        ...
        for (let i = 0; i < section.children.length; i++) {
            ...
            if (node.knownStackDiff) {
                ...
                //stop at pinned ops, same reasoning as the forward scan
                if (node.opinfo.pinned) { break; }
                let info = node.opinfo.stackinfo;
    ...
    //an inconsistent section only invalidates its own equation, don't let it abort
    //the entire callibration (which also prevented the callibration from ever
    //completing and being saved before)
    let failedequations = 0;
    for (let section of allsections) {
        ...
        try { testSection(eq); } catch (e) { failedequations++; }
    ...
    for (let i = 0; i < 3; i++) {
        ...
        console.log("total", total, "done", done, "partial", partial, "incomplete", missing.size, "eqfailed", failedequations);
    }
