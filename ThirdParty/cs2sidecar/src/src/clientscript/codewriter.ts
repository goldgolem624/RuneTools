...
import { ClientScriptOp, PrimitiveType, binaryOpSymbols, branchInstructionsOrJump, getOpName, int32MathOps, longJsonToBigInt, namedClientScriptOps, popDiscardOps, popLocalOps, pushConstantIntOp, subtypeToTs, subtypes } from "./definitions";
...
addWriter(RawOpcodeNode, (node, ctx) => {
    if (node.op.opcode == pushConstantIntOp && node.children.length == 0) {
        //pushes its immediate, render as the plain constant it is
        return String(node.op.imm);
    }
