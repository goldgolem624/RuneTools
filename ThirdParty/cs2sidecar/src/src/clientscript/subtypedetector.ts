...
const looseOps = [
    ...
    //int arithmetic is used on typed ids all the time (offsets into enum keys etc),
    //propagating exact types through it just produces conflicts
    ...[...int32MathOps].flatMap(q => [dependencyGroup("opin", q) | dependencyIndex("int", 0), dependencyGroup("opin", q) | dependencyIndex("int", 1), dependencyGroup("opout", q) | dependencyIndex("int", 0)]),
];
                ...
                if (stackinout) {
                    ...
                    if (stack) {
                        ...
                        if (index < typedstack.length) {
                            let known = typedstack[index];
                            //unknown_* placeholders carry no information, seeding them
                            //would conflict with real types solved through other paths
                            if (known != subtypes.unknown_int && known != subtypes.unknown_long && known != subtypes.unknown_string) {
                                this.knowntypes.set(key, known);
                            }
                        }
                    }
                }
...
export function assignKnownTypes(calli: ClientscriptObfuscation, knowntypes: Map<number, number>) {
    for (let op of calli.mappings.values()) {
        ...
        //fall back to a pre-existing exact type before unknown_*, hand-callibrated ops
        //(callibrator.ts postReferenceOps) declare types the solver can't derive
        let previn = op.stackinfo.exactin;
        let prevout = op.stackinfo.exactout;
        let exactin = new ExactStack();
        let diffin = op.stackinfo.in.getStackdiff();
        for (let i = 0; i < diffin.int; i++) { exactin.int.push(knowntypes.get(dependencyGroup("opin", op.id) | dependencyIndex("int", i)) ?? previn?.int[i] ?? subtypes.unknown_int); }
        for (let i = 0; i < diffin.long; i++) { exactin.long.push(knowntypes.get(dependencyGroup("opin", op.id) | dependencyIndex("long", i)) ?? previn?.long[i] ?? subtypes.unknown_long); }
        for (let i = 0; i < diffin.string; i++) { exactin.string.push(knowntypes.get(dependencyGroup("opin", op.id) | dependencyIndex("string", i)) ?? previn?.string[i] ?? subtypes.unknown_string); }
        op.stackinfo.exactin = exactin;

        let exactout = new ExactStack();
        let diffout = op.stackinfo.out.getStackdiff();
        for (let i = 0; i < diffout.int; i++) { exactout.int.push(knowntypes.get(dependencyGroup("opout", op.id) | dependencyIndex("int", i)) ?? prevout?.int[i] ?? subtypes.unknown_int); }
        for (let i = 0; i < diffout.long; i++) { exactout.long.push(knowntypes.get(dependencyGroup("opout", op.id) | dependencyIndex("long", i)) ?? prevout?.long[i] ?? subtypes.unknown_long); }
        for (let i = 0; i < diffout.string; i++) { exactout.string.push(knowntypes.get(dependencyGroup("opout", op.id) | dependencyIndex("string", i)) ?? prevout?.string[i] ?? subtypes.unknown_string); }
