...
//pushes its int immediate, kept outside namedClientScriptOps so the display name
//stays PUSH_CONSTANT_INT from rs3opnames
export const pushConstantIntOp = 10574;
...
export class StackConstants {
    ...
    popList(other: StackList, endoffset?: number) {
        //clamp, the tracker may hold fewer values than the real stack when it was
        //restarted after an op with unknown stack effect
        this.values.length = Math.max(0, this.values.length - other.total(endoffset));
    }
