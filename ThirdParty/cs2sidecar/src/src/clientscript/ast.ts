...
export class VarAssignNode extends AstNode {
    ...
    addVar(node: RawOpcodeNode) {
        ...
        //pops are encountered top-of-stack first, so this pop's consumed values sit
        //BELOW everything already collected, prepend to keep the in list in push
        //order (appending reversed it, which made every mixed type destructure fail
        //to match its pushed values)
        this.knownStackDiff.in.values.unshift(...getNodeStackIn(node).values);
    }
}
...
export function translateAst(ast: CodeBlockNode) {
    ...
    let expandNode = (node: AstNode) => {
        ...
        if (outtype.isEmpty()) {
            ...
            //exception: constants stay available, the jagex compiler regularly
            //interleaves unrelated void calls between the argument pushes of a later
            //consumer and capping the constants here would orphan them into stack()
            //statements (moving a constant across a statement can't change behavior).
            //local reads may cross a plain client op the same way since client ops
            //can't write locals, but not assignments or composed statements.
            //
            //this is only sound when we actually know what this node does with the
            //stack, an op with an unresolved signature (eg dbrow_getfield on a table
            //whose schema isn't in the cache) reports no inputs and no outputs while
            //really consuming the pending values and pushing a result. letting values
            //flow past one of those would silently drop its arguments and hand its
            //result to whatever reads next, so cap everything instead
            let effectKnown = true;
            if (node instanceof RawOpcodeNode) {
                let info = node.knownStackDiff ?? node.opinfo.stackinfo;
                effectKnown = !node.unknownstack && info.initializedthrough;
            }
            let allowLocalFlow = effectKnown && node instanceof RawOpcodeNode;
            let keptcount = 0;
            for (let entry of usablestackdata) {
                if (effectKnown && entry.stackconst != null && entry.stackel.children.length == 0) {
                    usablestackdata[keptcount++] = entry;
                    continue;
                }
                if (allowLocalFlow && entry.stackel instanceof RawOpcodeNode && entry.stackel.children.length == 0 && pushLocalOps.includes(entry.stackel.op.opcode)) {
                    usablestackdata[keptcount++] = entry;
                    continue;
                }
                let { stackel } = entry;
            ...
            usablestackdata.length = keptcount;
        } else {
    ...
    //values still pending at the end of the section are consumed by whatever runs
    //next, mark them as pushes so they can't be read as no-op statements
    for (let { stackel } of usablestackdata) {
        if (!stackel.parent) { continue; }
        let capnode = new ComposedOp(stackel.originalindex, "stack");
        stackel.parent.replaceChild(stackel, capnode);
        capnode.push(stackel);
    }
...
export function setRawOpcodeStackDiff(consts: StackConstants | null, calli: ClientscriptObfuscation, node: RawOpcodeNode) {
    ...
    } else if (node.opinfo.id == pushConstantIntOp) {
        //the pushed value is the int immediate, expose it as a constant so joins,
        //vararg strings and equations can use it like a pushconst
        node.knownStackDiff = StackInOut.fromExact([], [typeuuids.int++]);
        node.knownStackDiff.constout = node.op.imm;
    } else if (node.opinfo.id == namedClientScriptOps.dbrow_findnext) {
        //listed in dynamicOps but its stack shape is static, it pushes the next row of
        //the active query (primed by DB_FIND/db_find_with_count/DBQUERY_EXECUTE_COUNT)
        node.knownStackDiff = StackInOut.fromExact([], [subtypes.dbrow]);
    ...
    for (let node of children) {
        ...
        if (consts) {
            if (node.knownStackDiff?.constout != null) {
                ...
                //an op with unknown stack effect invalidates everything tracked so
                //far, but values pushed after it are still indexed correctly from the
                //top, so restart with an empty tracker instead of giving up on the
                //rest of the section (a lookup deeper than the fresh tracker returns
                //undefined and the dynamic op handlers skip safely)
