...
cmdts.runSafely(cmd, cliArguments()).then(() => {
	console.log("rendercli done");
	if (typeof window !== "undefined" && (window as any).onCliCompleted) (window as any).onCliCompleted(0);
}, (e) => {
	console.error("rendercli error", e);
	if (typeof window !== "undefined" && (window as any).onCliCompleted) (window as any).onCliCompleted(1);
})