...
// Jagex's packed-HSL -> RGB palette (Rasteriser.createPalette / JagexColor.createPalette).
// This is NOT the textbook HSL2RGBfloat below. Three differences, all of which matter:
//   * hue, saturation and lightness carry fixed offsets, and lightness divides by 128 not 127
//   * the result is raised to a brightness power (the client's brightness setting)
//   * the palette quantises to 1/256 steps, twice
// Omitting the brightness power makes every vertex colour too dark, which is what the icon
// renderer's old `pow(vcol, 0.8)` fudge was blindly compensating for.
// RSMV_HSL_BRIGHTNESS=1 disables the power (used when fitting the exponent).
// 0.6 is Jagex's BRIGHTNESS_MAX. Recovered independently here: dumping vertex colours with the
// power disabled and letting a global fit solve for the exponent lands on 0.565, and pinning it
// at 0.6 lets the albedo take its faithful value while the fitted ambient becomes neutral.
const HSL_BRIGHTNESS = +(process.env.RSMV_HSL_BRIGHTNESS ?? 0.6);

function hue2rgbJagex(p: number, q: number, t: number) {
	if (t < 0) { t += 1; }
	if (t > 1) { t -= 1; }
...
export function packedHSL2RGBfloat(packed: number, brightness = HSL_BRIGHTNESS): [number, number, number] {
	const hue = ((packed >> 10) & 0x3f) / 64 + 0.0078125;
	const sat = ((packed >> 7) & 0x07) / 8 + 0.0625;
	const lum = (packed & 0x7f) / 128;
	let r = lum, g = lum, b = lum;
	if (sat !== 0) {
		const q = lum < 0.5 ? lum * (1 + sat) : lum + sat - lum * sat;
		const p = 2 * lum - q;
		r = hue2rgbJagex(p, q, hue + 1 / 3);
		g = hue2rgbJagex(p, q, hue);
		b = hue2rgbJagex(p, q, hue - 1 / 3);
	}
	// the palette truncates to 1/256 before and after the brightness power
	const q8 = (v: number) => Math.min(255, Math.floor(v * 256)) / 256;
	return [
		Math.pow(q8(r), brightness),
		Math.pow(q8(g), brightness),
		Math.pow(q8(b), brightness),
	];
}

export function packedHSL2RGB(packed: number, brightness = HSL_BRIGHTNESS): [number, number, number] {
	const f = packedHSL2RGBfloat(packed, brightness);
	return [
		Math.min(255, Math.floor(f[0] * 256)),
		Math.min(255, Math.floor(f[1] * 256)),
		Math.min(255, Math.floor(f[2] * 256)),
	];
}
