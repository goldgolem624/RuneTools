import { packedHSL2HSL, HSL2RGB, packedHSL2RGB, ModelModifications, posmod, getOrInsert } from "../utils";
...
export function modifyMesh(mesh: ModelMeshData, mods: ModelModifications) {
	...
	let clonedhsl: BufferAttribute | undefined = undefined;
	// EXACT recolour path: when the mesh carries per-vertex packed HSL, op40 replacement is
	// a plain ushort compare (the engine's own semantics), immune to decoded-RGB key
	// collisions. The RGB attribute is rebuilt from the palette for replaced vertices.
	if (mods.replaceColors && mods.replaceColors.length != 0 && mesh.attributes.colorHsl && mesh.attributes.color) {
		let hsl = mesh.attributes.colorHsl;
		let colors = mesh.attributes.color;
		for (let i = 0; i < hsl.count; i++) {
			let cur = hsl.getX(i);
			for (let repl of mods.replaceColors) {
				if (cur == repl[0]) {
					...
					clonedhsl.setX(i, repl[1]);
					let [r, g, b] = packedHSL2RGB(repl[1]);
					clonedcolors!.setXYZ(i, r / 255, g / 255, b / 255);
					break;
				}
			}
		}
	} else if (mods.replaceColors && mods.replaceColors.length != 0 && mesh.attributes.color) {
		let colors = mesh.attributes.color;

		// The vertex colours were decoded with Jagex's real palette (packedHSL2RGB), so the
		// recolour keys must be built with the SAME decoder or nothing matches. Using the old
		// textbook HSL2RGB here silently broke every op40 recolour (bronze bar rendered gold,
		// etc.) once the palette was corrected.
		let map: [number, [number, number, number]][] = [];
		for (let repl of mods.replaceColors) {
			let oldcol = packedHSL2RGB(repl[0]);
			let newcol = packedHSL2RGB(repl[1]);
