import { packedHSL2HSL, HSL2RGB, packedHSL2RGB, ushortToHalf } from "../../utils";
...
function parsePosData(arr: Int16Array | Uint32Array) {
	// build 940 player rework stores positions as big-endian float32 (read as Uint32Array here);
	// byte-swap + reinterpret. Older models store int16 (Int16Array) used directly.
	if (arr instanceof Uint32Array) {
		let f = new Float32Array(arr.length);
		let dv = new DataView(new ArrayBuffer(4));
		for (let i = 0; i < arr.length; i++) { dv.setUint32(0, arr[i], true); f[i] = dv.getFloat32(0, false); }
...
export function parseOb3Model(modelfile: Buffer, source: CacheFileSource) {
	...
	if (parsed.meshes) {
		for (let mesh of parsed.meshes) {
			...
			if (mesh.colourBuffer) {
				...
				for (let i = 0; i < mesh.faceCount; i++) {
					let [r, g, b] = packedHSL2RGB(mesh.colourBuffer[i]);
		...
		if (mesh.vertexColours) {
			let vertexcolor = new Uint8Array(mesh.vertexCount * 4);
			let vertexhsl = new Uint16Array(mesh.vertexCount);
			let alphaBuffer = mesh.vertexAlpha;
			for (let i = 0; i < mesh.vertexColours.length; i++) {
				let [r, g, b] = packedHSL2RGB(mesh.vertexColours[i]);
				...
				vertexhsl[i] = mesh.vertexColours[i];
			}
			attributes.color = new THREE.BufferAttribute(vertexcolor, 4, true);
			attributes.colorHsl = new THREE.BufferAttribute(vertexhsl, 1);
		}
		...
		for (let render of mesh.renders) {
			...
			meshes.push({
				...
				// unkbyte2 behaves like the classic per-face render priority, per render group:
				// higher draws later (on top) regardless of depth. The anima-of-Bik disc
				// (122888) needs it to put the black core above the vein hub.
				// renders[].unkint behaves like the classic render priority: higher paints
				// later (on top). Verified consistent: anima-of-Bik translucent film = 8 over
				// core/veins = 6; potion liquid = 4/5 under glass = 6.
				priority: (render as any).unkint >>> 0,
				unkint: (render as any).unkint >>> 0,
