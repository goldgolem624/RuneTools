import { HSL2RGBfloat, packedHSL2HSL, packedHSL2RGBfloat } from "../../utils";
...
export function convertMaterial(data: Buffer, materialid: number, source: CacheFileSource) {
	...
	if (rawparsed.v0) {
		...
		if (raw.extra) {
			...
			// Jagex's palette, not the textbook HSL2RGB: it carries offsets and a brightness power.
			mat.baseColor = (raw.extra.baseColor == 0 ? [1, 1, 1] : packedHSL2RGBfloat(raw.extra.baseColor));
		}
