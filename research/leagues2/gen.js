...
md.push("  covers'). Unlock state = varp 12327, a locality BITMASK indexed by locality id");
md.push("  (script20136 tests one bit; script20133 requires all of a region's bits).");
md.push("  It is a 64-BIT varp: bits 0-31 come back from the ordinary varp read, bits");
md.push("  32-40 from the node's wide read (launcher varpsLong). SOLVED 2026-08-11 from a");
md.push("  live capture: varp 12327 = 0x8000001C_08AC2046, where");
md.push("    low  0x08AC2046 = bits 1,2,6,13,18,19,21,23,27");
md.push("    high 0x8000001C = a 0x80000000 MARKER BIT (not data) + bits 34,35,36");
md.push("  Clearing the marker resolves all eleven regions exactly as the game shows them");
md.push("  (unlocked: Karamja, Tirannwn, Misthalin, Havenhythe).");
md.push("  TWO TRAPS, both hit live before the capture settled it:");
md.push("    * varp 12328 is NOT the bit-32 continuation - it reads 0.");
md.push("    * Number() on the raw i64 string loses the low bits (19 digits vs ~16 of");
md.push("      precision), and any double >= 2^54 is even, so testing bits on one double");
md.push("      returns 0 for EVERY bit. Keep the halves separate and use integer ops:");
md.push("      low from the 32-bit read, high = floor(unsigned/2^32) % 2^31 (that floor");
md.push("      survives the rounding, since the error is far below 2^32).");
md.push("  First unlock slot is always Karamja (script21081).");
